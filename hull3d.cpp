...
void timerfunc(int value) {
  ...
  if (animate_mode) {
    if (hull.size()) {
      hull_index_render = (hull_index_render+1)%hull.size();
    }
...
void draw_hull(vector<triangle3d>& hull){
  ...
  for (int i=0; i< N; i++) {
    ...
    //draw the triangle
    //print_triangle(hull[i], "drawing: ", "\n");
    glBegin(GL_TRIANGLES);
    glVertex3i(hull[i].a->x, hull[i].a->y,hull[i].a->z);
    glVertex3i(hull[i].b->x, hull[i].b->y,hull[i].b->z);
    glVertex3i(hull[i].c->x, hull[i].c->y,hull[i].c->z);
    glEnd();
  }//for 
}//draw-hull 
